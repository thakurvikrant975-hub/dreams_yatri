// prisma/seed.ts — Dreams Yatri complete seed
...
for (const r of teamRoles) {
  ...
  // ── 5. ACTIVITIES — pricing columns flattened onto activities table ────────
  const actShikara = await db.activities.create({
    data: {
      name: "Dal Lake Shikara Ride", slug: "dal-lake-shikara-ride", destination_id: kashmir.id,
      duration_hours: 2.0, difficulty: "easy", category: "sightseeing",
      price: 800, original_price: 1000, margin_percentage: 12,
      pricing_type: "per_person", min_persons: 1, max_persons: null,
      description: "Traditional shikara boat ride on iconic Dal Lake.",
      meta_title: "Dal Lake Shikara Ride | Dreams Yatri", meta_desc: "Book shikara ride in Srinagar.",
    }
  });

  const actGondola = await db.activities.create({
    data: {
      name: "Gulmarg Gondola Ride", slug: "gulmarg-gondola-ride", destination_id: kashmir.id,
      duration_hours: 3.0, difficulty: "easy", category: "adventure",
      price: 1500, original_price: 1800, margin_percentage: 15,
      pricing_type: "per_person", min_persons: 1, max_persons: null,
      description: "Asia's highest gondola with Himalayan views.",
      meta_title: "Gulmarg Gondola | Dreams Yatri", meta_desc: "Book Gulmarg Gondola Phase 1 and 2.",
    }
  });

  const actSkiing = await db.activities.create({
    data: {
      name: "Skiing at Gulmarg", slug: "skiing-at-gulmarg", destination_id: kashmir.id,
      duration_hours: 5.0, difficulty: "moderate", category: "adventure",
      price: 3000, original_price: 4000, margin_percentage: 18,
      pricing_type: "per_person", min_persons: 1, max_persons: null,
      description: "World-class skiing on Gulmarg's Himalayan slopes.",
      meta_title: "Skiing Gulmarg | Dreams Yatri", meta_desc: "Book skiing at Gulmarg.",
    }
  });

  const actMughal = await db.activities.create({
    data: {
      name: "Mughal Gardens Tour", slug: "mughal-gardens-tour", destination_id: kashmir.id,
      duration_hours: 4.0, difficulty: "easy", category: "sightseeing",
      price: 600, original_price: 800, margin_percentage: 12,
      pricing_type: "per_person", min_persons: 1, max_persons: null,
      description: "Guided tour of Shalimar Bagh, Nishat Bagh.",
      meta_title: "Mughal Gardens Tour | Dreams Yatri", meta_desc: "Book Mughal gardens tour.",
    }
  });

  const actKufri = await db.activities.create({
    data: {
      name: "Kufri Snow Activities", slug: "kufri-snow-activities", destination_id: himachal.id,
      duration_hours: 4.0, difficulty: "easy", category: "adventure",
      price: 1500, original_price: 1800, margin_percentage: 15,
      pricing_type: "per_person", min_persons: 1, max_persons: null,
      description: "Snow activities at Kufri — skiing, yak rides, toboggan.",
      meta_title: "Kufri Snow Activities | Dreams Yatri", meta_desc: "Book Kufri snow activities.",
    }
  });

  const actRohtang = await db.activities.create({
    data: {
      name: "Rohtang Pass Day Trip", slug: "rohtang-pass-day-trip", destination_id: himachal.id,
      duration_hours: 8.0, difficulty: "easy", category: "sightseeing",
      price: 2500, original_price: 3200, margin_percentage: 15,
      pricing_type: "per_person", min_persons: 1, max_persons: null,
      description: "Full day excursion to Rohtang Pass at 3978m.",
      meta_title: "Rohtang Pass Trip | Dreams Yatri", meta_desc: "Book Rohtang Pass excursion.",
    }
  });

  console.log(`✅ Activities: 6 → shikara(${actShikara.id}) gondola(${actGondola.id}) skiing(${actSkiing.id}) mughal(${actMughal.id}) kufri(${actKufri.id}) rohtang(${actRohtang.id})`);
  ...
  await db.activity_images.createMany({
    data: [
      ...
      { activity_id: actGondola.id, url: "https://images.unsplash.com/photo-1604537529428-15bcbeecfe4d?w=1200", thumbnail: "https://images.unsplash.com/photo-1604537529428-15bcbeecfe4d?w=400", alt: "Gulmarg snow peak", sort_order: 2, is_primary: false },
  ...
  const pkg1 = await db.packages.create({
    data: {
      title: "Kashmir Grand Tour", slug: "kashmir-grand-tour",
      destination_id: kashmir.id,
      description: "Paradise on earth — houseboat stays on Dal Lake, snow adventures in Gulmarg and verdant valleys of Pahalgam.",
      meta_title: "Kashmir Grand Tour | Dreams Yatri",
      meta_desc: "Book Kashmir Grand Tour — Dal Lake, Gulmarg and Pahalgam. From ₹14,999.",
      thumbnail: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400",
      cover_image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=1920",
      is_active: true,
    }
  });
  ...
  await db.package_hotels.createMany({
    data: [
  ...
  await db.package_activities.createMany({
    data: [
  ...
  await db.package_tags.createMany({
    data: [
      { package_id: pkg1.id, tag_id: tagSnow.id },
      { package_id: pkg1.id, tag_id: tagAdventure.id },
      { package_id: pkg1.id, tag_id: tagFamily.id },
      { package_id: pkg1.id, tag_id: tagHoneymoon.id },
      { package_id: pkg1.id, tag_id: tagLuxury.id },
    ]
  });
  await db.package_categories.createMany({
    data: [
  ...
  const pkg2 = await db.packages.create({
    data: {
      title: "Shimla Manali Classic", slug: "shimla-manali-classic",
      ...
      meta_title: "Shimla Manali Tour Package | Dreams Yatri",
      meta_desc: "Book Shimla Manali Classic — Kufri, Solang and Rohtang Pass. From ₹12,999.",
      thumbnail: "https://images.unsplash.com/photo-1558618047-f4e60cef0008?w=400",
      cover_image: "https://images.unsplash.com/photo-1558618047-f4e60cef0008?w=1920",
      is_active: true,
    }
  });
  ...
  await db.package_itineraries.createMany({
    data: [
      // ── 5-day ──
      { package_id: pkg2.id, duration_id: p2d5.id, day: 1, route_index: null, hotel_id: hotelSnow.id, hotel_days: 2, title: "Chandigarh to Shimla", description: "Pickup from Chandigarh, scenic drive to Shimla. Check-in, Mall Road evening.", activity_ids: [], activities: ["Chandigarh pickup", "Mall Road walk", "Check-in"], meals: ["Dinner"] },
      ...
      // ── 7-day route-1 (Delhi) ──
      { package_id: pkg2.id, duration_id: p2d7.id, day: 1, route_index: 1, hotel_id: hotelSnow.id, title: "Delhi to Shimla", description: "Drive from Delhi via Kalka to Shimla.", activity_ids: [], activities: ["Delhi pickup", "Kalka-Shimla drive", "Check-in"], meals: ["Dinner"] },
      { package_id: pkg2.id, duration_id: p2d7.id, day: 2, route_index: 1, hotel_id: hotelSnow.id, title: "Shimla & Kufri", description: "Snow activities at Kufri and Shimla sightseeing.", activity_ids: [actKufri.id], activities: ["Kufri snow activities", "Jakhu Temple", "Christ Church"], meals: ["Breakfast", "Dinner"] },
      { package_id: pkg2.id, duration_id: p2d7.id, day: 3, route_index: 1, hotel_id: hotelManu.id, title: "Shimla to Manali", description: "Drive through Kullu Valley to Manali.", activity_ids: [], activities: ["Kullu Valley drive", "Manali check-in"], meals: ["Breakfast", "Dinner"] },
      { package_id: pkg2.id, duration_id: p2d7.id, day: 4, route_index: 1, hotel_id: hotelManu.id, title: "Solang Valley", description: "Adventure day at Solang Valley.", activity_ids: [], activities: ["Solang Valley ropeway", "Snow activities", "Zorbing"], meals: ["Breakfast", "Dinner"] },
      { package_id: pkg2.id, duration_id: p2d7.id, day: 5, route_index: 1, hotel_id: hotelManu.id, title: "Manali Sightseeing", description: "Hadimba Temple, Vashisht springs, Tibetan market.", activity_ids: [], activities: ["Hadimba Temple", "Vashisht hot springs", "Tibetan market"], meals: ["Breakfast", "Dinner"] },
      { package_id: pkg2.id, duration_id: p2d7.id, day: 6, route_index: 1, hotel_id: hotelManu.id, title: "Rohtang Pass Excursion", description: "Day excursion to Rohtang Pass at 3978m.", activity_ids: [actRohtang.id], activities: ["Rohtang Pass", "Snow photography", "Yak ride"], meals: ["Breakfast", "Dinner"] },
      { package_id: pkg2.id, duration_id: p2d7.id, day: 7, route_index: 1, hotel_id: null, title: "Departure to Delhi", description: "Post-breakfast check out. Drive back to Delhi.", activity_ids: [], activities: ["Breakfast", "Drive to Delhi"], meals: ["Breakfast"] },
    ]
  });
  ...
  await db.package_tags.createMany({
    data: [
      { package_id: pkg2.id, tag_id: tagSnow.id },
      { package_id: pkg2.id, tag_id: tagAdventure.id },
      { package_id: pkg2.id, tag_id: tagFamily.id },
      { package_id: pkg2.id, tag_id: tagBudget.id },
  ...
  const labels = [
    ...
    "packages              ", "package_durations      ", "package_stay_categories", "package_pricing        ",
    "package_itineraries   ", "hotels                 ", "hotel_room_pricing     ", "hotel_images           ",
