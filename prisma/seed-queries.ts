...
async function seedQueries() {
  ...
  await db.packageQuery.deleteMany(); // clear existing test data
  ...
  const dummyQueries = [
    ...
    },
    {
      ...
      message: "Goa. Group of 20. Need comfortable buses and hotels. Need welocome drink",
  ...
  for (const q of dummyQueries) {
    await db.packageQuery.create({ data: q });
  }
