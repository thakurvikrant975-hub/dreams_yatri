...
// Booking confirmation (deposit)
{
    ...
    check("confirmation shows deposit paid", m.html.includes("Deposit paid") && m.html.includes("₹9,113.35"));
    check("confirmation shows balance + due", m.html.includes("₹27,340.03") && m.html.includes("Balance due") && m.html.includes("2026"));
...
// Booking confirmation (full) → no balance row
{
    ...
    check("full shows paid in full", m.html.includes("Paid in full") && m.html.includes("₹50,000.00"));
...
// Cancellation
{
    ...
    check("cancellation shows refund + fee", m.html.includes("₹8,202.02") && m.html.includes("₹911.33"));
}

// Refund confirmed
{
    const m = refundConfirmedEmail({ bookingNumber: "DY-3", packageTitle: "Manali", refundAmountPaise: 820202 });
    check("refund subject has amount", m.subject.includes("₹8,202.02") && m.subject.includes("DY-3"));
    check("refund body has amount", m.html.includes("₹8,202.02"));
}
...
// Ops
{
    ...
    check("ops body has amount + pax", m.html.includes("₹9,113.35") && m.html.includes("Travellers"));
}
