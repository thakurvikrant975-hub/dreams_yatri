// This module is deliberately free of React and of the "use client" directive.
//
// It used to live at the bottom of doc-theme.tsx, alongside the DocTheme
// context — which made the whole file a client module, and every export in it a
// client reference. Server code that only wanted `isTemplateId` (the itinerary
// settings action, validating a template id read out of the database) got a
// proxy instead of a function and threw at call time. Keeping the data, the
// predicates and the resolver on this side of that boundary means both a Server
// Action and a Client Component can use them.
//
// doc-theme.tsx re-exports everything here, so client code can keep importing
// from either. Server code must import from THIS file.
