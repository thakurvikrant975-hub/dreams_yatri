...
export function CreateTeamMemberDialog({ departments, roles, onCreated }: Props) {
  ...
  const reset = () => {
    setForm({ name: "", email: "", password: "", departmentId: "", roleId: "", joiningDate: "", isActive: true });
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        // Capture before reset — these are needed for the password dialog
        const plain = form.password;
        const name = form.name;
        const id = result.data.id;
        setOpen(false);
        reset();
        // Fire callback → opens PasswordDialog in TeamMembersTable
        onCreated(id, name, plain);
      } else {
