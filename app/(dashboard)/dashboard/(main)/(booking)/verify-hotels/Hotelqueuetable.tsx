...
import {
  ...
  CalendarDays, Users, Hotel, RefreshCw,
} from "lucide-react";
...
export function HotelQueueTable({ paginated, currentPage }: Props) {
  ...
  const handleConfirm = () => {
    ...
    start(async () => {
      const r = await confirmHotel(actionBooking.booking.id, notes || undefined);
      if (r.success) {
        toast.success("Hotel confirmed — moved to cab verification");
  ...
  const handleFlag = () => {
    ...
    start(async () => {
      const r = await flagHotelIssue(actionBooking.booking.id, notes);
      if (r.success) {
    ...
    <>
      ...
      <Dialog open={actionBooking?.type === "confirm"} onOpenChange={(o) => !o && setActionBooking(null)}>
        <DialogContent className="sm:max-w-[440px]">
          <DialogHeader>
            <DialogTitle>Confirm Hotel — {actionBooking?.booking.bookingNumber}</DialogTitle>
            <DialogDescription>
              Confirm hotel availability and pricing for this booking.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={actionBooking?.type === "flag"} onOpenChange={(o) => !o && setActionBooking(null)}>
        <DialogContent className="sm:max-w-[440px]">
          <DialogHeader>
            <DialogTitle>Flag Hotel Issue — {actionBooking?.booking.bookingNumber}</DialogTitle>
            <DialogDescription>Describe the hotel availability or pricing issue.</DialogDescription>
          </DialogHeader>
          <div className="space-y-2 py-2">
