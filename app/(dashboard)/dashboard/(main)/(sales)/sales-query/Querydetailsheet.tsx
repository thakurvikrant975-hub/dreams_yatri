...
import {
    ...
    CalendarClock, XCircle, Globe, RotateCcw,
...
function timelineDot(event: string) {
    if (event.includes("✅") || event.includes("Converted")) return "bg-emerald-500";
...
export function SalesQueryDetailSheet({
    ...
    // Render nothing if no query — but keep the Sheet so it can animate closed
    const isEmpty = !query;

    const status = query?.status as QueryStatus | undefined;
    const active = status ? isActiveQuery(status) : false;
    const closed = status ? isClosedQuery(status) : false;

    // FIX: safely access _count — it's always included now from getSalesQueryById
    // but we guard with fallback to avoid any runtime TypeError
    const followUpCount = query?._count?.queryFollowUps ?? 0;
    const reqs = query?.requirements as PackageRequirements | null | undefined;
                ...
                {isEmpty ? (
                    ...
                    <>
                        <SheetHeader className="px-6 pt-6 pb-4 border-b">
                            ...
                            {/* Action buttons — active queries */}
                            {active && (
                                <div className="flex gap-2 pt-3 flex-wrap">
                                    <PackageDetailsDialog
                                        query={query as any}
                                        initialRequirements={reqs ?? null}
                                        onDone={onRefresh}
                                    >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {followUpCount > 0
                                                ? `Update Follow-Up (${followUpCount})`
                                                : "Add Follow-Up"}
                                        </Button>
                                    </AddFollowUpDialog>
                        ...
                        <ScrollArea className="flex-1">
                            <div className="px-6 py-4 space-y-6">
                                ...
                                {/* Follow-Ups — only current user's entries */}
                                <section>
                                    <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-3">
                                    ...
                                    <div className="space-y-2 mb-3">
                                        {(query.followUps ?? []).length === 0 ? (
                                            <p className="text-xs text-muted-foreground italic">
                                                No follow-ups yet — only your own follow-ups appear here.
                                            </p>
                                        ) : (
