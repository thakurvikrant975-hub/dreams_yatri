...
import { getPackageHistory } from "../actions";

// ── Types ─────────────────────────────────────────────────────────────────────

type HistoryEntry = Awaited<ReturnType<typeof getPackageHistory>>[number];
    ...
    function handleOpen(o: boolean) {
        ...
        if (o && !loaded) {
            startTransition(async () => {
                const result = await getPackageHistory(id);
                setHistory(result);
                setLoaded(true);
            });
        }
            ...
            <SheetContent side="right" className="w-full sm:max-w-md flex flex-col">
                <SheetHeader className="shrink-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                        Full audit trail of changes made to this package
                    </p>
                </SheetHeader>
                ...
                <div className="flex-1 overflow-y-auto mt-4">
                    {isPending ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-16 gap-3 text-muted-foreground">
                            ...
                            <p className="text-xs">Actions on this package will appear here</p>
                        </div>
                    ) : (
                        <div className="px-1">
                            {history.map((entry, i) => (
