...
import { Bed, Clock, MapPin, PenLine, User } from "lucide-react";
...
export function TeamHotelRequestsTable({
    requests, stats, currentPage, totalPages, totalCount, limit, search, scopeLabel,
}: {
    ...
    const columns: ColumnDef<HotelRequestRow>[] = [
        ...
        },
        {
            ...
            width: "w-[130px]",
            cell: (r) => (
                // Opens the package builder itself, not the Hotel Department's
                // fill form — a Team Leader/Sales Manager corrects the day's
                // requirements or notes if that's what's blocking the request,
                // rather than filling the hotel in themselves.
                <Link
                    href={`/dashboard/package-builder/${r.id}`}
                    target="_blank"
                    rel="noopener"
                    title="Open in the editor to correct the day's requirements"
                    className="inline-flex items-center gap-1 rounded-md border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-xs font-semibold text-indigo-700 transition-colors hover:bg-indigo-100"
                >
    ...
    return (
        <div className="space-y-6">
            <p className="text-xs text-dashboard-base-content/50">Showing hotel requests for <span className="font-medium text-dashboard-base-content">{scopeLabel}</span></p>
            ...
            <DataTable
                ...
                emptyState={
                    <TableEmptyState
                        ...
                        description="Packages appear here when someone on your team flags a day as needing a hotel from the team."
                    />
                }
                pagination={{
                    currentPage,
