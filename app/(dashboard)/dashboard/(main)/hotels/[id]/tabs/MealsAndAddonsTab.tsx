...
import {
  ...
  CalendarDays, Info, UtensilsCrossed, ShoppingBag,
} from "lucide-react";
import {
...
import {
  ...
  type HotelAddon, type AddonInput, type AddonSeasonInput,
} from "../../actions";
...
// ── Shared season types & components ─────────────────────────────────────
...
function SeasonsInlineList({
  ...
  const calendarSeasons: SeasonRange[] = seasons
    ...
    .map((x) => ({
      from: x.valid_from.slice(5), to: x.valid_to.slice(5),
...
type MealFormState = {
  meal_type: string; label: string;
  price: string; weekend_price: string;
  is_active: boolean; seasons: SeasonEntry[];
};
...
function toMealFormState(m: HotelMealPricing): MealFormState {
  return {
    meal_type: m.meal_type, label: m.label,
    price: String(m.price),
    weekend_price: m.weekend_price ? String(m.weekend_price) : "",
    is_active: m.is_active,
    seasons: m.seasons.map((s) => ({
      tempId: uid(),
      valid_from: toISODate(s.valid_from), valid_to: toISODate(s.valid_to),
...
function buildMealInput(form: MealFormState): MealPricingInput {
  return {
    meal_type: form.meal_type, label: form.label.trim(),
...
function MealForm({
  initial, onSave, onCancel, isSaving, usedMealTypes = new Set<string>(),
}: {
  initial: MealFormState; onSave: (form: MealFormState) => void;
  onCancel: () => void; isSaving: boolean; usedMealTypes?: Set<string>;
}) {
    ...
    <div className="border border-dashboard-base-content/20 rounded-xl p-4 space-y-4 bg-dashboard-base-200/60 shadow-sm">
        ...
        >
          ...
          <SelectContent>
            {PRESET_MEALS.map((m) => {
              ...
              return (
                <SelectItem
                  key={m.value} value={m.value} disabled={alreadyUsed}
      ...
      {form.meal_type === "CUSTOM" && (
        <div className="space-y-1.5">
          ...
          <Input
            placeholder="e.g. Evening High Tea" value={form.label}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          ...
          <Input type="number" min={0} placeholder="150" value={form.price}
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5 h-5">
            <input type="checkbox" id="meal-weekend" checked={!!form.weekend_price}
          ...
          {form.weekend_price !== "" && (
            <Input type="number" min={0} placeholder="Same as weekday" value={form.weekend_price}
      ...
      <div className="border-t border-dashboard-base-content/10 pt-4">
        <SeasonsInlineList seasons={form.seasons} onChange={(s) => upd("seasons", s)} basePrice={Number(form.price) || 0} />
      </div>
...
function MealCard({ meal, onEdit, onDelete, isDeleting }: {
  meal: HotelMealPricing; onEdit: () => void; onDelete: () => void; isDeleting: boolean;
}) {
    ...
    <div className="border border-dashboard-base-content/20 rounded-xl overflow-hidden bg-dashboard-base-100 shadow-sm">
      ...
      {open && meal.seasons.length > 0 && (
        <div className="px-4 pb-4 pt-2 border-t border-dashboard-base-content/10 space-y-2">
          <p className="text-[10px] font-bold uppercase tracking-widest text-dashboard-base-content/50 mb-2">Seasonal Pricing</p>
          {meal.seasons.map((s) => (
            <div key={s.id} className="flex items-center justify-between text-xs bg-dashboard-base-200 rounded-lg px-3 py-1.5">
              ...
              <span className="font-medium text-dashboard-base-content">
                ...
                {s.weekend_price && (
                  <span className="text-dashboard-base-content/50 ml-1.5">· ₹{s.weekend_price.toLocaleString("en-IN")} wknd</span>
                )}
              </span>
            </div>
          ))}
...
type AddonFormState = {
  label: string; description: string; charge_type: string;
  price: string; weekend_price: string;
  is_active: boolean; seasons: SeasonEntry[];
};
  ...
  return {
    ...
    seasons: form.seasons
      ...
      .map((s) => ({
        ...
        weekend_price: s.weekend_price ? Number(s.weekend_price) : null,
        is_active: true,
      } satisfies AddonSeasonInput & { season_name: string })),
  };
}
...
function AddonForm({
  ...
  initial: AddonFormState; onSave: (form: AddonFormState) => void;
  ...
  const overlaps = overlappingIds(form.seasons);
  const isValid =
    !!form.label.trim() && !!form.charge_type && !!form.price && Number(form.price) > 0 &&
    overlaps.size === 0 &&
    form.seasons.every((s) => !s.valid_from || (s.valid_from && s.valid_to && Number(s.price) > 0));
    ...
    <div className="border border-dashboard-base-content/20 rounded-xl p-4 space-y-4 bg-dashboard-base-200/60 shadow-sm">
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          ...
          <Input
            placeholder="e.g. Porter Service, Bonfire, Gala Dinner"
            value={form.label} onChange={(e) => upd("label", e.target.value)} autoFocus
            className="bg-dashboard-base-100 border-dashboard-base-content/20"
          />
        </div>
        <div className="space-y-1.5">
          ...
          <Select value={form.charge_type} onValueChange={(v) => upd("charge_type", v)}>
            ...
            <SelectContent>
              {CHARGE_TYPES.map((ct) => (
                <SelectItem key={ct.value} value={ct.value} className="cursor-pointer">
                  {ct.label}
                </SelectItem>
              ))}
            </SelectContent>
      ...
      <div className="space-y-1.5">
        <Label className="text-sm font-medium text-dashboard-base-content">Description</Label>
        <Input
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          ...
          <Input type="number" min={0} placeholder="500" value={form.price}
            onChange={(e) => upd("price", e.target.value)}
            className="bg-dashboard-base-100 border-dashboard-base-content/20"
          />
          <p className="text-xs text-dashboard-base-content/40">
            {form.charge_type === "PER_PERSON" ? "Per person" :
             form.charge_type === "PER_ROOM" ? "Per room" : "Per booking"}
          </p>
        </div>
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5 h-5">
            <input type="checkbox" id="addon-weekend" checked={!!form.weekend_price}
              onChange={(e) => upd("weekend_price", e.target.checked ? form.price : "")}
              className="h-3.5 w-3.5 accent-dashboard-primary cursor-pointer"
            />
...
function AddonCard({ addon, onEdit, onDelete, isDeleting }: {
  addon: HotelAddon; onEdit: () => void; onDelete: () => void; isDeleting: boolean;
}) {
  const [open, setOpen] = useState(false);
  const chargeLabel = CHARGE_TYPES.find((ct) => ct.value === addon.charge_type)?.label ?? addon.charge_type;
  ...
  return (
    <div className="border border-dashboard-base-content/20 rounded-xl overflow-hidden bg-dashboard-base-100 shadow-sm">
      <div className="flex items-center justify-between px-4 py-3 hover:bg-dashboard-base-200 transition-colors">
        <button type="button" onClick={() => setOpen((p) => !p)}
          ...
          {open
            ? <ChevronDown className="h-4 w-4 text-dashboard-base-content/40 shrink-0" />
            : <ChevronRight className="h-4 w-4 text-dashboard-base-content/40 shrink-0" />}
          <ShoppingBag className="h-4 w-4 text-dashboard-secondary/60 shrink-0" />
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <p className="text-sm font-semibold text-dashboard-base-content truncate">{addon.label}</p>
              <Badge variant="outline" className="text-[10px] px-1.5 py-0 border-dashboard-base-content/20">
                {chargeLabel}
              </Badge>
              {!addon.is_active && <Badge variant="secondary" className="text-[10px] px-1.5 py-0">Inactive</Badge>}
            </div>
            <p className="text-xs text-dashboard-base-content/50">
              ₹{addon.price.toLocaleString("en-IN")} weekday
              {addon.weekend_price ? ` · ₹${addon.weekend_price.toLocaleString("en-IN")} weekend` : ""}
              {addon.seasons.length > 0 ? ` · ${addon.seasons.length} season${addon.seasons.length !== 1 ? "s" : ""}` : ""}
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This will permanently remove this add-on and all its seasonal rates.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel className="cursor-pointer">Cancel</AlertDialogCancel>
      ...
      {open && addon.seasons.length > 0 && (
        <div className="px-4 pb-4 pt-2 border-t border-dashboard-base-content/10 space-y-2">
          <p className="text-[10px] font-bold uppercase tracking-widest text-dashboard-base-content/50 mb-2">Seasonal Rates</p>
          {addon.seasons.map((s) => (
            <div key={s.id} className="flex items-center justify-between text-xs bg-dashboard-base-200 rounded-lg px-3 py-1.5">
              <span className="text-dashboard-base-content/60">{s.season_name}</span>
              <span className="font-medium text-dashboard-base-content">
...
export function MealsAndAddonsTab({
  hotel_id, initialMeals, initialAddons,
}: {
  hotel_id:      number;
      ...
      setMeals((prev) => [
        ...prev,
        {
          ...
          seasons: form.seasons
            .filter((s) => s.valid_from && s.valid_to && Number(s.price) > 0)
            .map((s, i) => ({
              id: Date.now() + i, meal_pricing_id: res.id!,
              season_name: `${fmtMonthDay(s.valid_from)} → ${fmtMonthDay(s.valid_to)}`,
              valid_from: new Date(s.valid_from), valid_to: new Date(s.valid_to),
              price: Number(s.price),
              weekend_price: s.weekend_price ? Number(s.weekend_price) : null,
  ...
  function handleEditMeal(id: number, form: MealFormState) {
    startMealTransition(async () => {
      ...
      setMeals((prev) => prev.map((m) =>
        m.id !== id ? m : {
          ...m, meal_type: form.meal_type, label: form.label.trim(),
          price: Number(form.price),
          weekend_price: form.weekend_price ? Number(form.weekend_price) : null,
          is_active: form.is_active,
          seasons: form.seasons
            .filter((s) => s.valid_from && s.valid_to && Number(s.price) > 0)
            .map((s, i) => ({
              id: Date.now() + i, meal_pricing_id: id,
              season_name: `${fmtMonthDay(s.valid_from)} → ${fmtMonthDay(s.valid_to)}`,
              valid_from: new Date(s.valid_from), valid_to: new Date(s.valid_to),
              price: Number(s.price),
              weekend_price: s.weekend_price ? Number(s.weekend_price) : null,
  ...
  function handleEditAddon(id: number, form: AddonFormState) {
    startAddonTransition(async () => {
      ...
      setAddons((prev) => prev.map((a) =>
        a.id !== id ? a : {
          ...a, label: form.label.trim(), description: form.description.trim() || null,
          charge_type: form.charge_type,
          price: Number(form.price),
          weekend_price: form.weekend_price ? Number(form.weekend_price) : null,
          is_active: form.is_active,
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-dashboard-base-content flex items-center gap-2">
              <UtensilsCrossed className="h-4 w-4 text-dashboard-primary/70" />
              Meal Pricing
            </p>
            <p className="text-xs text-dashboard-base-content/50 mt-0.5">
              Per-person meal supplements with optional seasonal &amp; weekend rates.
            </p>
          </div>
          {!addingMeal && (
            <Button size="sm"
      ...
      <div className="space-y-5 bg-dashboard-base-100 p-8 rounded-xl shadow-lg border border-dashboard-base-content/20">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-dashboard-base-content flex items-center gap-2">
              <ShoppingBag className="h-4 w-4 text-dashboard-secondary/70" />
              Add-ons &amp; Extras
            </p>
            <p className="text-xs text-dashboard-base-content/50 mt-0.5">
              Optional charges (porter, bonfire, gala dinner…) billed per person, per room, or per booking.
            </p>
          </div>
          {!addingAddon && (
            <Button size="sm" variant="outline"
              className="gap-1.5 border-dashboard-base-content/30 text-dashboard-base-content hover:bg-dashboard-base-200 cursor-pointer"
        ...
        {addons.length === 0 && !addingAddon ? (
          <div className="flex flex-col items-center justify-center py-12 rounded-xl border border-dashed border-dashboard-base-content/20 bg-dashboard-base-200/30">
            <ShoppingBag className="h-8 w-8 text-dashboard-base-content/20 mb-3" />
