...
import { ShieldCheck, Clock, CheckCircle2, XCircle, Send, Eye, MessageSquare, Pencil } from "lucide-react";
    ...
    const columns: ColumnDef<PackageRow>[] = [
        ...
        },
        {
            ...
            cell: (p) => {
                ...
                return (
                    // Two ways into the same review, on purpose. The left button
                    // is the established screen costing uses today; the right one
                    // opens the same package in the editor, where corrections are
                    // made where they sit. Both stay until the new one has been
                    // run against real packages for long enough to replace the
                    // old one, and until then neither is hidden behind the other.
                    //
                    // New tab for both: opening one leaves the queue exactly as
                    // the reviewer left it — same page, same filter, same scroll
                    // — to come back to. `rel="noopener"` rather than
                    // "noreferrer", because the editor's back control reads
                    // document.referrer to return here.
                    <div className="inline-flex items-center gap-1.5 justify-end">
                        <Link
                            ...
                            rel="noopener"
                            title={actionable
                                ? "Review on the classic screen"
                                : "Back with the sales exec — nothing to review until they resubmit it"}
