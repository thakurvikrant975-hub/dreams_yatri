...
import {
    Ban, Check, CheckCheck, ChevronDown, ChevronUp,
    Clock, ClipboardList, MapPin, Phone, StickyNote, Save, XCircle,
} from "lucide-react";
...
import {
    approveReopenRequest, rejectReopenRequest, updateReopenRequestReviewNote,
    type ReopenRequestRow, type ReopenRequestsFilter, type ReopenRequestStats,
} from "./actions";
...
/** A reviewer jotting a note on a request while it's still in the queue —
 * saved independently of approve/reject so it isn't lost if they want to
 * think it over first. Mirrors lead-requests/LeadRequestsTable.tsx's
 * ReviewNote. */
function ReviewNote({ request }: { request: ReopenRequestRow }) {
    ...
    function save() {
        startTransition(async () => {
            const result = await updateReopenRequestReviewNote(request.id, note);
            if (result.success) {
    ...
    return (
        <div className="rounded-lg border border-dashboard-base-300 bg-dashboard-base-200/30 p-2.5 space-y-2">
            ...
            <Textarea
                ...
                placeholder="Note for the requester or whoever picks this up…"
                rows={2}
...
function RequestDetailPanel({ request }: { request: ReopenRequestRow }) {
    const router = useRouter();
    const [isPending, startTransition] = useTransition();
    ...
    function approve() {
        startTransition(async () => {
            const result = await approveReopenRequest(request.id);
            if (result.success) {
                toast.success(`${request.query?.name ?? "Query"} reopened`);
                router.refresh();
            } else {
    ...
    function reject() {
        ...
        startTransition(async () => {
            const result = await rejectReopenRequest(request.id, reason);
            if (result.success) {
                toast.success(`Reopen request for ${request.query?.name ?? "the query"} rejected`);
                setRejecting(false);
                setReason("");
                router.refresh();
            } else {
        ...
        <div className="px-4 py-3.5 space-y-2.5 bg-dashboard-base-200/20">
            ...
            {request.status === "APPROVED" && (
                <p className="text-xs text-emerald-700 dark:text-emerald-400 flex items-center gap-1">
                    <Check className="h-3 w-3" /> Approved by {request.decidedByName}
                </p>
            )}
...
export function ReopenRequestsTable({
    requests,
    stats,
    currentPage,
    totalPages,
    ...
    requests: ReopenRequestRow[];
    stats: ReopenRequestStats;
    currentPage: number;
    totalPages: number;
    totalCount: number;
    limit: number;
    ...
    const columns: ColumnDef<ReopenRequestRow>[] = [
        {
            header: "Query",
            sortKey: (r) => r.query?.name?.toLowerCase() ?? "",
            cell: (r) => (
                <div>
                    <span className="font-semibold text-sm text-dashboard-base-content line-clamp-1">
                        {r.query?.name ?? "Query no longer exists"}
                    </span>
                    {r.query?.destination && (
                        <div className="mt-0.5 flex items-center gap-1 text-[11px] text-dashboard-neutral">
                            <MapPin className="size-3 shrink-0" /> {r.query.destination}
                        </div>
                    )}
                    {r.query?.phone && (
                        <div className="mt-0.5 flex items-center gap-1 text-xs text-dashboard-base-content">
                            <Phone className="size-3 text-dashboard-neutral shrink-0" /> {r.query.phone}
                        </div>
                    )}
                </div>
            ),
        },
        {
            header: "Reason",
            cell: (r) => (
                <p className="text-xs text-dashboard-base-content/80 line-clamp-2 max-w-xs" title={r.reason}>
                    {r.reason}
                </p>
            ),
        },
                ...
                <div className="flex flex-col items-center gap-1">
                    ...
                    {r.status === "REJECTED" && r.rejectionReason && (
                        <span
                            ...
                            className="text-[10px] text-dashboard-base-content/45 max-w-27.5 truncate"
                        >
                            {r.rejectionReason}
                        </span>
                    )}
                        ...
                        {r.status === "PENDING" && (
                            <Button
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    startTransition(async () => {
                                        const result = await approveReopenRequest(r.id);
                                        if (result.success) {
                                            toast.success(`${r.query?.name ?? "Query"} reopened`);
                                            router.refresh();
                                        } else {
                                            toast.error(result.error ?? "Failed to approve");
                                        }
    ...
    return (
        <div className="space-y-6">
            ...
            <StatGrid cols={4}>
                ...
                <StatCard label="Approved" value={stats.approved} icon={CheckCheck} />
            ...
            <DataTable
                ...
                rowKey={(r) => r.id}
                rowClassName={(r) =>
                    r.status === "APPROVED" ? "bg-emerald-50/40 hover:bg-emerald-50" :
                ...
                emptyState={
                    <TableEmptyState
                        title="No reopen requests found"
                        description="Requests appear here once a sales exec asks to reopen a closed query."
                    />
                }
                pagination={{
                    currentPage,
