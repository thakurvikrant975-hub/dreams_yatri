...
import { fetchRegionPackages, type RegionPackageItem, type RegionPackagesPage } from '@/app/actions/regions/fetch-region-page'

interface Props {
    regionId: number
    initial: RegionPackagesPage
}

export default function RegionPackagesList({ regionId, initial }: Props) {
    const router = useRouter()

    const [items, setItems]     = useState<RegionPackageItem[]>(initial.items)
    ...
    const loadMore = useCallback(async () => {
        ...
        setError(false)
        try {
            const next = await fetchRegionPackages(regionId, page + 1, initial.pageSize)
    ...
    }, [loading, hasMore, regionId, page, initial.pageSize])
        ...
        return (
            <div className="py-16 text-center">
                ...
                <p className="text-sm text-secondary mt-1">
                    We&apos;re still curating trips for this region. Check back soon.
                </p>
            </div>
        )
    }
