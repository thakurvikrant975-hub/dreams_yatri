...
function ItinaryHeaderSkelton() {
    return (
        <div className='bg-white rounded-2xl ring-1 ring-(--border-default) w-full'>
            <div className='flex gap-2 overflow-x-auto px-3.5 py-2.5 no-scrollbar bg-neutral-200/80 rounded-t-[inherit]'>
                {TABS.map(({ id, label, icon: Icon }) => (
                    <button
                        key={id}
                        type="button"
                        className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-full ring-[0.1em] ring-inset text-[13px] font-medium whitespace-nowrap transition-all duration-150 font-heading opacity-50 pointer-events-none', id === 'Plan'
            ...
            <div className='py-4 px-3.5'>
                <Skeleton className='h-6 w-full mb-3.5 rounded-full' />
                <Skeleton className='h-5 w-3/4 mb-2 rounded-full' />
                <Skeleton className='h-5 w-3/4 mb-2 rounded-full' />
                <Skeleton className='h-6 w-full mb-2 rounded-full mt-3' />
                <Skeleton className='h-5 w-1/4 mb-2 rounded-full mt-3' />
                <Skeleton className='h-6 w-full mb-2 rounded-full mt-3' />
                <Skeleton className='h-3.5 w-1/2 mb-2 rounded-full' />
                <Skeleton className='h-3.5 w-1/2 mb-2 rounded-full' />
                <Skeleton className='h-3.5 w-1/2 mb-2 rounded-full' />
            </div>
