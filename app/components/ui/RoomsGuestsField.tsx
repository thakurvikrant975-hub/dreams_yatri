...
import { summarizeRoomGuests, type RoomGuests } from '@/app/lib/packages/roomGuests';

const MAX_CHILDREN_PER_ROOM = 10;
const CHILD_AGE_MAX = 11; // children = below 12 years
const MAX_ROOM_CARDS = 20; // UI ceiling — independent of real hotel inventory
...
export interface RoomsGuestsFieldProps {
    value: RoomGuests[];
    onChange: (rooms: RoomGuests[]) => void;
    /** Real inventory ceiling (the package page has one; search surfaces don't). */
    maxRooms?: number;
    /** How many guests one room sleeps. Defaults to unconstrained, which is the
     *  honest answer on search surfaces — no hotel has been chosen yet. */
    personsPerRoom?: number;
    /** Package page only: before rejecting an over-capacity request, try to
     *  find a same-category hotel that can cover it. Resolves true if unlocked. */
    onRequestMoreRooms?: (needed: number) => Promise<boolean>;
    /** z-index utility for the popover. */
    menuZClass?: string;
    id?: string;
    className?: string;
}
...
/**
 * MMT-style rooms & guests picker: each room is configured independently, with
 * its own Adults/Children and per-child ages, and the whole draft is committed
 * on Apply.
 *
 * Shared by the package page (where it's bounded by real hotel inventory) and
 * the search surfaces (home hero, /packages search bar), so the split a guest
 * makes while searching is the same shape the package page then prices.
 */
export default function RoomsGuestsField({
    value, onChange, maxRooms = MAX_ROOM_CARDS, personsPerRoom = Infinity,
    onRequestMoreRooms, menuZClass = 'z-100', id, className,
}: RoomsGuestsFieldProps) {
    const [open, setOpen] = useState(false);

    // Draft edited inside the popover; committed on Apply (mirrors MMT).
    const [draftRooms, setDraftRooms] = useState<RoomGuests[]>(value);
    const [expandedIndex, setExpandedIndex] = useState(0);
    const [checkingAvailability, setCheckingAvailability] = useState(false);

    // Reset the draft as the popover opens rather than in an effect, so there's
    // no render where the draft still shows the previous selection.
    ...
    async function apply() {
        ...
        if (draftRooms.length > maxRooms) {
            ...
            if (onRequestMoreRooms) {
                setCheckingAvailability(true);
                const unlocked = await onRequestMoreRooms(draftRooms.length);
        ...
        <Popover.Root open={open} onOpenChange={onOpenChange}>
            <Popover.Trigger asChild>
                ...
                >
                    ...
                    <span className="flex-1 truncate text-sm text-neutral-800">{summarizeRoomGuests(value)}</span>
            ...
            <Popover.Portal>
                <Popover.Content
                    ...
                    avoidCollisions
                    collisionPadding={12}
                    className={cn(menuZClass, 'w-[min(92vw,380px)] max-h-(--radix-popover-content-available-height) overflow-y-auto overscroll-contain rounded-xl border border-neutral-200 bg-white p-4 shadow-xl shadow-black/10')}
                >
                    ...
                    <div className="mt-4 flex items-center justify-between border-t border-neutral-100 pt-3">
                        <span className="text-xs text-neutral-400">
                            {checkingAvailability ? 'Checking room availability…' : summarizeRoomGuests(draftRooms)}
                        </span>
                        <Button
                            variant="premium"
                            size="sm"
