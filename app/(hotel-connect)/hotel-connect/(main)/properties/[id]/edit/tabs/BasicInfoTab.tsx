...
import type { BasicInfoState, BasicInfoValues } from "./basic-info-schema";
...
const CHANNEL_MANAGERS = [
  "1HotelConnect", "Aiosell", "APerfectStay", "Arpies Yazilim", "ASI Front Desk",
  "AsiaTech Inc", "Autoupdate-PMS", "availpro", "Axisrooms", "Beds24", "Billzify",
  "Bizweb", "Book Better", "BookandLink", "BookingHotel", "bookingjini",
  "Bookingsmaker", "Bookito", "BookLogicMaxiChannel", "BookOne Channel Manager",
  "Botshot", "channelmanager com au", "Channex", "Cloudbeds", "Cubilis",
  "Dataman Computer System Pvt Ltd", "Derby", "Dingus", "e-Bridge Travel",
  "EaseRoom", "eglobe solutions", "EzeeCentrix", "Faircloud PMS",
  "Fastbooking DM", "FastrackbookingCRS", "Hireavilla Website",
  "Hotel Link Solutions", "Hotel-Spider", "Hotelcloud", "HoteliersGuru",
  "HotellineCM", "HotelRunner", "hotelsolutions", "Hoteltopic", "HPECOSOC",
  "HyperGuest", "iConnect", "Instaconnekt", "Maximojo", "Mini Hotel Man",
  "NextPax", "Octorate", "oHotels.in", "OTASWITCH", "Oyo",
  "perfecthandssolutions", "primalRES", "ProExsus", "ProfitRoomCHM",
  "ProHotelConnect", "QloApps", "RATEBOTAI", "Rategain", "Ratetiger",
  "Reddoorz", "Rentals United", "RentalWise", "ResAvenue", "Reseliva",
  "RevCatalyst", "RevCHM", "RMS", "RoomcloudChannel", "Saasaro", "SANYUKT",
  "sfconnect", "Shiji Group", "Simplified Management", "SiteMinder",
  "SiteMinder-EMEA", "SkyChannels", "SmartHotel", "Spektum", "STAAH",
  "Staymaker CM", "Suyog", "SyncRooms", "Synxis", "TravelClick", "Travelline",
  "TripMakerz", "UP Tours", "Update247", "Vertical Booking CRS", "Weazy",
  "Xmitter", "YashRaj Softwares", "YieldPlanet ChannelManager",
];
...
const STAR_OPTIONS = [
  { value: "0", label: "Non-Rated / Budget" },
...
export default function BasicInfoTab({ hotel }: { hotel: HotelBasicInfo }) {
  ...
  const [channelMgr,    setChannelMgr]    = useState(hotel.channel_manager_name ?? "");
  ...
  // Pre-verified when the field already has a saved value (DB = previously verified)
  ...
  function clearError(field: keyof BasicInfoValues) {
  ...
  async function handleSendEmailOtp() {
    ...
    if (!res.ok) {
      setEmailOtpErr(res.error ?? "Failed to send OTP");
      return;
    }
    ...
    if (res.devOtp) {
      ...
      setEmailOtp(res.devOtp); // auto-fill in dev
    }
  }
  ...
  async function handleVerifyEmailOtp() {
    ...
    setEmailLoading(false);
    if (!res.ok) {
      setEmailOtpErr(res.error ?? "Verification failed");
      return;
    }
  ...
  function validate(data: FormData): NonNullable<BasicInfoState["fieldErrors"]> {
    const errs: NonNullable<BasicInfoState["fieldErrors"]> = {};
    ...
    // Year of construction can't be after bookings started
  ...
  return (
    <form id="wizard-form" action={formAction} onSubmit={handleSubmit} className="space-y-5">
      ...
      <SectionCard title="Property Details">
        ...
        >
          <SearchSelect
            ...
            // Resolve cross-field error if the new since-year is no longer before year_built
      ...
      <SectionCard
        ...
        desc="Keeps your room availability in sync across all connected booking platforms automatically"
      >

        <FieldRow label="Connected to a channel manager?">
          <div className="flex items-center gap-6 h-10">
            <label className="flex items-center gap-2.5 text-sm text-neutral-800 cursor-pointer select-none">
              <input
                ...
                onChange={() => setHasChannelMgr(false)}
                disabled={isPending}
            ...
            <label className="flex items-center gap-2.5 text-sm text-neutral-800 cursor-pointer select-none">
              <input
                ...
                value="on"
                checked={hasChannelMgr}
                onChange={() => setHasChannelMgr(true)}
                disabled={isPending}
                className="h-4 w-4 accent-primary-500 cursor-pointer"
              />
              Yes
        ...
        {hasChannelMgr && (
          <FieldRow label="Channel Manager Name" error={fe.channel_manager_name}>
            <SearchSelect
              ...
              options={CHANNEL_MANAGERS}
              value={channelMgr}
              onChange={(v) => { setChannelMgr(v); clearError("channel_manager_name"); }}
              placeholder="Select channel manager"
              searchPlaceholder="Search channel manager..."
              disabled={isPending}
              error={!!fe.channel_manager_name}
            />
          </FieldRow>
        )}
      ...
      >
        ...
        {/* ── Landline ── */}
        <FieldRow label="Landline / Alternate Number (Optional)" error={fe.contact_landline}>
          <Input
